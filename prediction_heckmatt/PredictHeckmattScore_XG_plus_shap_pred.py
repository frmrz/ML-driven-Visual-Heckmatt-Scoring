...
def generate_shap_plots_for_class(dfOut_skf, X_test, dataDir, result_dir, explainer, feature_names, observations, ii, class_num,seed=123456, link_f='logit'):
    ...
    if link_f == 'logit':
        imgPIL.save(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'logit', img_filename))
    else:
        imgPIL.save(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'odds', img_filename))
    ...
    if link_f == 'logit':
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'logit', beeswarm_plot_filename), dpi=300, bbox_inches='tight')
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'logit', beeswarm_plot_filename_pdf), bbox_inches='tight')  # Save as PDF for vector format
    else:
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'odds', beeswarm_plot_filename), dpi=300, bbox_inches='tight')
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'odds', beeswarm_plot_filename_pdf), bbox_inches='tight')
    ...
    # append the overall effect of the non-top features to the top features names, with the name "Sum of 29 Other features"
    top_features = np.append(top_features, "Sum of 29 Other features")
    ...
    new_base_value = exp_val_prob[class_num-1]
    base_value = explainer.expected_value[class_num-1]
    
    # plt.figure()
    # shap.decision_plot(base_value, 
    #                 top_features_shap_values.values, 
    ...
    # decision_plot_filename = f'ShapDecisionPlotCorrectClass{class_num}_{ii}.png'
    
    # plt.savefig(os.path.join(result_dir, 'SHAP', decision_plot_filename), dpi=300, bbox_inches='tight')
    # plt.clf()
    ...
    if link_f == 'logit':
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'logit', decision_plot_filename), dpi=300, bbox_inches='tight')
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'logit', decision_plot_filename_pdf), bbox_inches='tight')
    else:
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'odds', decision_plot_filename), dpi=300, bbox_inches='tight')
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'odds', decision_plot_filename_pdf), bbox_inches='tight')
    ...
    if link_f == 'logit':
        ax[1].imshow(plt.imread(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'logit', beeswarm_plot_filename)))
    else:   
        ax[1].imshow(plt.imread(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'odds', beeswarm_plot_filename)))
    ax[1].axis('off')
    
    if link_f == 'logit':
        ax[2].imshow(plt.imread(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'logit', decision_plot_filename)))
    else:
        ax[2].imshow(plt.imread(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'odds', decision_plot_filename)))
    ax[2].axis('off')
    ...
    if link_f == 'logit':
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'logit', final_plot_filename_png), dpi=300, bbox_inches='tight')
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'logit', final_plot_filename_pdf), bbox_inches='tight')
    else:
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'odds', final_plot_filename_png), dpi=300, bbox_inches='tight')
        plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'odds', final_plot_filename_pdf), bbox_inches='tight')
...
plt.savefig(os.path.join(result_dir, 'CorrCSA_pred.png'), bbox_inches='tight')

# save as svg
plt.savefig(os.path.join(result_dir, 'CorrCSA_pred.svg'), bbox_inches='tight')
plt.show()
...
plt.title('Correlation Matrix')
plt.savefig(os.path.join(result_dir, 'CorrLOW.png'), bbox_inches='tight')
plt.savefig(os.path.join(result_dir, 'CorrLOW.svg'), bbox_inches='tight')
plt.show()
...
dfX.to_csv(os.path.join(excel_dir, 'preProcessedDF_pred.csv'),index=False)
...
for ii, (train_index, test_index) in enumerate(skf.split(X, Y, groups)):
    ...
    if link_f == 'logit':
        images = [Image.open(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'logit', filename)) for filename in filenames]
    else:
        images = [Image.open(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'odds', filename)) for filename in filenames]
    ...
    # save the new image
    if link_f == 'logit':
        new_im.save(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'logit', f'CorrectClass_{ii}.png'))
    else:
        new_im.save(os.path.join(result_dir, 'SHAP_pred', 'FULL', 'odds', f'CorrectClass_{ii}_odds.png'))
    ...
    #     input_pdf_path = os.path.join(result_dir, 'SHAP', filename)
    ...
    # # Save the output PDF
    # output_path = os.path.join(result_dir, 'SHAP', f'CorrectClass_{ii}.pdf')
...
plt.tight_layout()
plt.savefig(os.path.join(result_dir, 'Top20Features_pred.png'), bbox_inches='tight')
plt.savefig(os.path.join(result_dir, 'Top20Features_pred.svg'), bbox_inches='tight')
plt.show()
...
for class_index in range(n_classes):
    ...
    plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', f'ShapSummaryPlot_{class_index}.png'), bbox_inches='tight')
    # clear the plot
    plt.show()
    plt.clf()
    ...
    # write the plot to a file
    plt.savefig(os.path.join(result_dir, 'SHAP_pred', 'FULL', f'ShapBeeswarmPlot_{class_index}.png'), bbox_inches='tight')
    # clear the plot
    # plt.show()
    plt.clf()
...
plt.tight_layout()
plt.savefig(os.path.join(result_dir, 'ConfusionMatrix_pred.png'), bbox_inches='tight')
plt.savefig(os.path.join(result_dir, 'ConfusionMatrix_pred.svg'), bbox_inches='tight')
...
plt.savefig(os.path.join(result_dir, 'RocOVR_pred.png'), bbox_inches='tight')
plt.savefig(os.path.join(result_dir, 'RocOVR_pred.svg'), bbox_inches='tight')
plt.show()
...
ax.legend(loc="lower right")
plt.savefig(os.path.join(result_dir, 'RocOVO_pred.png'), bbox_inches='tight')
plt.savefig(os.path.join(result_dir, 'RocOVO_pred.svg'), bbox_inches='tight')
plt.show()
...
plt.tight_layout()
# save the plot
plt.savefig(os.path.join(result_dir, 'BoxplotEIvsH_pred.png'), bbox_inches='tight')
plt.savefig(os.path.join(result_dir, 'BoxplotEIvsH_pred.svg'), bbox_inches='tight')
plt.show()
...
plt.tight_layout()
# save the plot
plt.savefig(os.path.join(result_dir, 'BoxplotEIvsH_predicted_pred.png'), bbox_inches='tight')
plt.savefig(os.path.join(result_dir, 'BoxplotEIvsH_predicted_pred.svg'), bbox_inches='tight')
plt.show()
...
correlations_df.to_csv(os.path.join(excel_dir, 'CorrelationEIvsH_pred.csv'), index=False)
